...
fn test_path_element_serialisation() {
    ...
    assert_eq!(
        json,
        serialised,
        "Path element should have serialised correctly"
    );

    let deserialised: PathElement =
        serde_json::from_str(json.as_ref()).expect("Deserialisation failed");
    assert_eq!(
        path_element,
        deserialised,
        "Deserialised element should match initial value"
    );
}
...
fn test_blob_serialisation() {
    ...
    assert_eq!(
        expected,
        serialised,
        "Serialisation should match expected value"
    );

    let deserialised: Blob =
        serde_json::from_str(serialised.as_str()).expect("Deserialisation failed");
    assert_eq!(
        blob,
        deserialised,
        "Deserialised blob should match initial value"
    );
}
...
fn test_entity_deserialisation() {
    ...
    let file = File::open(d.as_ref() as &Path).expect("Could not open test file");
    let deserialised: Entity = serde_json::from_reader(file).expect("Deserialisation failed");

    let expected_time = Utc.ymd(2017, 09, 21).and_hms(05, 41, 33);
    ...
    assert_eq!(
        expected,
        deserialised,
        "Deserialised value should match expectations"
    );
}
